...
(function(root) {
	...
	try{
		...
		var Target = $("#sewise-subtitles-js").parent();
		/*Target.append(tmpHtml);
		var rndElement = document.getElementById(_rndID);
		$(rndElement).remove();
		rndElement = null;*/
		var queryData = parseURL($("#sewise-subtitles-js").attr("src"));
		toTryProcess = false;
	}
	...
	playerElement.className = '_player_layout';
	var subtitlesElement = document.createElement('div');
	subtitlesElement.className = '_subtitles_layout';
	
	if(toTryProcess){
		Target.appendChild(playerElement);
		Target.appendChild(subtitlesElement);
	}else{
		Target.append(playerElement);
		Target.append(subtitlesElement);
	}
	...
	TSubtitlesShowed.prototype = {
		init: function(data) {
			...
			_ost.options = {
				...
				data: [], //必须
				target: null, //必须
				highlight: '_subtitles_highlight', //高亮的类名
	...
	var getSubtitleData = function(sourceid, lang, callback) {
		//http://192.168.1.24/service/playerapi/?do=getsubtitle&sourceid=pibpHUuQ;
		var serverPath;
		if(queryData.debug === "true"){
			queryData.porthost = "192.168.1.24";
			sourceid = queryData.recordid;
		}
		//console.log("porthost: " + queryData.porthost);
		serverPath = 'http://' + queryData.porthost + '/service/playerapi/?do=getsubtitle';
		//console.log("serverPath:" + serverPath);
		
		if(window.SewiseSubtitlesData){
			callback(window.SewiseSubtitlesData.words, window.SewiseSubtitlesData);
			return;
		}
		...
		$.ajax({
			...
			error:function() {
				//console.log('getSubtitleData Faild!');
			}
		});
	}
	// 重建字幕数据 
			...
			oSubtitlesShow.init({
				...
				'htmlEndEvent': function() {
					...
					$(Target).find('span').click(function() {
						...
						if (SewisePlayer && SewisePlayer.doSeek && time) {
...
(function(root) {
	if (root.document) {
		var styleText = 'span._subtitles_highlight{ background-color: #000; color: #4f1; } p._subtitles_highlight{ background-color: #E8E8E8; color: inherit; } ._player_layout{ height: 100%; width: 60%; float: left; } ._subtitles_layout{ float: right; height: 100%; width: 36%; overflow: auto; } ';
